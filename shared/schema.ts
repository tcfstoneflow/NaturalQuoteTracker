...
export const users = pgTable("users", {
  ...
  role: text("role").notNull().default("sales_rep"), // "admin", "sales_rep", "inventory_specialist", "vendor"
...
// Vendors table for vendor user role
export const vendors = pgTable("vendors", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  companyName: text("company_name").notNull(),
  businessType: text("business_type").notNull(), // "quarry", "fabricator", "distributor", "supplier"
  taxId: text("tax_id"),
  website: text("website"),
  description: text("description"),
  specialties: text("specialties").array(), // Array of specialties like ["granite", "marble", "quartz"]
  ...
  annualRevenue: text("annual_revenue"), // Stored as string for flexibility
  ...
  overallRating: integer("overall_rating").default(0), // 1-5 star rating
  qualityRating: integer("quality_rating").default(0),
  deliveryRating: integer("delivery_rating").default(0),
  serviceRating: integer("service_rating").default(0),
...
// Vendor products - products that vendors can supply
export const vendorProducts = pgTable("vendor_products", {
  id: serial("id").primaryKey(),
  vendorId: integer("vendor_id").notNull().references(() => vendors.id, { onDelete: 'cascade' }),
  productId: integer("product_id").notNull().references(() => products.id, { onDelete: 'cascade' }),
  supplierPrice: text("supplier_price").notNull(), // Price vendor charges us
  minimumOrderQuantity: integer("minimum_order_quantity").default(1),
  leadTimeDays: integer("lead_time_days").default(0),
  availability: text("availability").default("available"), // "available", "limited", "discontinued"
  notes: text("notes"),
  isPreferred: boolean("is_preferred").default(false), // Preferred vendor for this product
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const purchaseOrderItems = pgTable("purchase_order_items", {
  id: serial("id").primaryKey(),
  purchaseOrderId: integer("purchase_order_id").notNull().references(() => vendorPurchaseOrders.id, { onDelete: 'cascade' }),
  productId: integer("product_id").notNull().references(() => products.id),
  quantity: integer("quantity").notNull(),
...
// Extended vendor types
export type VendorWithDetails = Vendor & {
  user: User;
  vendorProducts: (VendorProduct & { product: Product })[];
  purchaseOrders: VendorPurchaseOrder[];
};

export type VendorPurchaseOrderWithDetails = VendorPurchaseOrder & {
  vendor: Vendor;
