...
async function importSlabRow(row: any, mapping: Record<string, string>, tx: any) {
  ...
  app.get("/api/workflow-instances/my", requireAuth, async (req, res) => {
    try {
      const userId = req.user!.id;
      const instances = await storage.getUserWorkflowInstances(userId);
      res.json(instances);
    } catch (error) {
      console.error("Get user workflow instances error:", error);
      res.status(500).json({ error: "Failed to fetch user workflow instances" });
    }
  });
  ...
  app.post("/api/workflow-instances", requireAuth, async (req, res) => {
    try {
      const instanceData = insertWorkflowInstanceSchema.parse({
        ...req.body,
        startedBy: req.user!.id
      });
      
      const instance = await storage.createWorkflowInstance(instanceData);
      
      // Broadcast workflow creation notification
      broadcastNotification({
        type: 'workflow_started',
        message: `New workflow "${instanceData.instanceName || 'Workflow'}" started`,
        timestamp: new Date().toISOString(),
        metadata: {
          instanceId: instance.id,
          workflowId: instance.workflowId,
          startedBy: req.user!.firstName + ' ' + req.user!.lastName
        }
      });
  ...
  app.put("/api/workflow-instances/:id", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const instance = await storage.updateWorkflowInstance(id, req.body);
      res.json(instance);
    } catch (error) {
  ...
  app.post("/api/workflow-step-instances/:id/complete", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { output, notes } = req.body;
      
      const stepInstance = await storage.completeWorkflowStep(id, output, notes);
      
      // Broadcast step completion notification
      broadcastNotification({
        type: 'workflow_step_completed',
        message: `Workflow step completed by ${req.user!.firstName} ${req.user!.lastName}`,
        timestamp: new Date().toISOString(),
        metadata: {
          stepInstanceId: id,
  ...
  // Workflow Templates
  app.get("/api/workflow-templates", requireAuth, async (req, res) => {
    try {
  ...
  app.post("/api/workflow-templates/:id/create-instance", requireAuth, async (req, res) => {
    try {
      ...
      const instance = await storage.createWorkflowFromTemplate(templateId, instanceData);
      res.status(201).json(instance);
    } catch (error) {
  ...
  // Workflow Comments
  app.get("/api/workflow-instances/:id/comments", requireAuth, async (req, res) => {
    try {
      const workflowInstanceId = parseInt(req.params.id);
      const comments = await storage.getWorkflowComments(workflowInstanceId);
      res.json(comments);
    } catch (error) {
      console.error("Get workflow comments error:", error);
      res.status(500).json({ error: "Failed to fetch workflow comments" });
    }
  });

  app.post("/api/workflow-instances/:id/comments", requireAuth, async (req, res) => {
    try {
      const workflowInstanceId = parseInt(req.params.id);
      const commentData = insertWorkflowCommentSchema.parse({
        ...req.body,
        workflowInstanceId,
      ...
      res.status(500).json({ error: "Failed to create workflow comment" });
    }
  });
